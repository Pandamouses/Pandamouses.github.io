...
## How to run
This model runs in python, after clicking `run file`, it will pop up a menu. And click `Model` in the menu bar, click `Run model` the model will start to run. Sheep will move randomly and wolves will move faster. There will be 3 wolves with random color **black, green** and **blue**, and all the sheep(40) are **white**. Wolves and sheep will only move 10 times. If sheep is within 20 unit range of wolf, the sheep would be caught and die. This is a [screen shot](hunting.png)of the model showing 3 dead sheep. If the model keeps running, then all the sheep is suppose to die.
